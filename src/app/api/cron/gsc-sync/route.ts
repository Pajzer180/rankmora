...
import { syncAllEligibleSearchConsoleProjects } from '@/lib/server/gsc/ingest';
import { RouteError, toRouteErrorResponse } from '@/lib/server/routeError';
import type { SearchConsoleCronSyncResponse } from '@/types/searchConsole';
...
export async function GET(req: Request) {
  try {
    ...
    const result = await syncAllEligibleSearchConsoleProjects();

    return NextResponse.json(result satisfies SearchConsoleCronSyncResponse);
  } catch (error) {
